...
from opytimizer.core.optimizer import (
...
class _BaseRunner:
    ...
    def __init__(
        ...
        self.sync_optimizer = (
            sync_method if callable(sync_method) else (lambda *args, **kwargs: None)
        )
    ...
    def metrics(self, m: Optional[Union[List[BaseMetric], BaseMetric]]) -> None:
        if isinstance(m, BaseMetric):
            ...
            raise e.TypeError(
                "`metrics` should be an instance or a list of `BaseMetric` instances."
            )
    ...
    def _ensure_space(
        ...
        is_tensorized_optimizer = isinstance(
            optimizer, (TensorizedOptimizer, TensorizedMultiObjectiveOptimizer)
        )
        is_tensorized_space = isinstance(
            space, (_SingleObjectiveTensorSpace, _MultiObjectiveTensorSpace)
        )
...
class _MultiObjectiveRunner(_BaseRunner):
    ...
    def _compute_metrics(self) -> Dict:
        ...
        return {
            metric.name.lower(): metric(self.space.pareto_front)
    ...
    def _set_postfix(
        self, stopping_vessel: _StoppingVessel, metric_values: Dict, **kwargs
    ) -> None:
...
class Opytimizer:
    def __new__(
        ...
        runner_cls = (
            _SingleObjectiveRunner if space.n_objectives == 1 else _MultiObjectiveRunner
        )
