...
class Environment:
    ...
    def __init__(
        self, device: Literal["numpy", "cupy"] = "numpy", dtype: str = "float32"
    ):
    ...
    def set_backend(self, backend: Literal["numpy", "cupy"] | Backend) -> Environment:
        ...
        try:
            ...
            logger.info(
                "The current device does not have CuPy installed. Backend switched to numpy."
            )
    ...
    def __repr__(self) -> str:
        return (
            f"Environment(backend={self._backend.value!r}, " f"dtype={self._dtype!r} "
        )
