    ...
    **kwargs,
    ...
    if need_matrix:
        ...
        if F.wants(fmap, "obj_labels"):
            out["obj_labels"] = kwargs.get("obj_labels") or [
    ...
    if F.wants(fmap, "alg_labels"):
        out["alg_labels"] = kwargs.get("labels") or [
            f"Algorithm {i+1}" for i in range(len(args))
        ]
    if F.wants(fmap, "colors"):
        out["colors"] = plt.colormaps.get_cmap("tab10")(np.linspace(0, 1, len(args)))
