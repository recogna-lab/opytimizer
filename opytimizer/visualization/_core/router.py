...
from opytimizer.visualization._core.renderers import (
    _RENDERER_REGISTRY,
    BaseRenderer,
    MatplotlibRenderer,
)
...
class PlotRouter:
    ...
    def dispatch(
        self,
        plot_fn_mpl: Callable,
        plot_fn_ply: Callable,
        data: Dict,
        ...
        **kwargs,
        ...
        plot_fn = (
            plot_fn_mpl if isinstance(renderer, MatplotlibRenderer) else plot_fn_ply
        )
        data = {**data, **kwargs}
    ...
    def _resolve_backend(self, backend: str | type) -> BaseRenderer:
        if isinstance(backend, BaseRenderer):
            return backend
        if isinstance(backend, type) and issubclass(backend, BaseRenderer):
            return backend()
