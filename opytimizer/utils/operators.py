...
class BaseCrossover(ABC):
    ...
    def __init__(
        self,
        rate: float = 1.0,
        return_mode: Literal["first", "second", "both", "random"] = "both",
    ) -> None:
        self.rate = rate
    ...
    def _return_offspring(self, children: List[Agent]) -> Union[Agent, List[Agent]]:
        ...
        if self.return_mode == "first":
            return [children[0]]
        elif self.return_mode == "second":
            return [children[1]]
        elif self.return_mode == "both":
            return children
        else:
            return [children[np.random.choice([0, 1])]]
    ...
    def return_mode(self, value: Literal["first", "second", "both", "random"]) -> None:
        ...
        if value not in allowed:
            ...
            raise ValueError(
                f"`return_mode` should be one of {allowed}, but got '{value}'."
            )
...
class ArithmeticCrossover(ContinuousCrossover):
    ...
    def __call__(self, parent1: Agent, parent2: Agent) -> tuple:
        ...
        if self.rate < np.random.random():
            ...
            child1.position[mask] = (
                alpha[mask] * x1[mask] + (1 - alpha[mask]) * x2[mask]
            )
            child2.position[mask] = (
                alpha[mask] * x2[mask] + (1 - alpha[mask]) * x1[mask]
            )
...
class GaussianMutation(BaseMutation):
    ...
    def __call__(self, agent: Agent) -> Agent:
        ...
        if np.any(mask):
            ...
            mutant.position = new_position
            mutant.clip_by_bound()
...
class SBXCrossover(ContinuousCrossover):
    ...
    def __call__(self, parent1: Agent, parent2: Agent) -> tuple:
        ...
        if child1.position.ndim == 1:
            child1.position = child1.position.reshape(-1, 1)
        if child2.position.ndim == 1:
            child2.position = child2.position.reshape(-1, 1)
        ...
        betaq1[~mask_b1] = np.power(
            (1.0 / (2.0 - rand[~mask_b1] * alpha1[~mask_b1])), exponent
        )
        ...
        betaq2[~mask_b2] = np.power(
            (1.0 / (2.0 - rand[~mask_b2] * alpha2[~mask_b2])), exponent
        )
...
class PolynomialMutation(BaseMutation):
    ...
    def __call__(self, agent: Agent) -> Agent:
        ...
        if np.any(mask):
            ...
            if np.any(lower):
                xy = 1.0 - delta1[lower]
                val = 2.0 * rand[lower] + (1.0 - 2.0 * rand[lower]) * np.power(
                    xy, (self.eta + 1.0)
                )
            ...
            if np.any(upper):
                xy = 1.0 - delta2[upper]
                val = 2.0 * (1.0 - rand[upper]) + 2.0 * (rand[upper] - 0.5) * np.power(
                    xy, (self.eta + 1.0)
                )
