        ...
        if r.generate_uniform_random_number() < self.obl_rate:
            ...
            for agent in space.agents:
                ...
                if strategy == "basic":
                    ...
                    opposite_pos = self.get_quasi_opposite_position(
                        agent.position, space
                    )
                elif strategy == "generalized":
                    opposite_pos = self.get_generalized_opposite_position(
                        agent.position, space
                    )
                elif strategy == "partial":
                    opposite_pos = self.get_partial_opposite_position(
                        agent.position, space
                    )
                elif strategy == "center":
                    opposite_pos = self.get_center_opposite_position(
                        agent.position, space
                    )
                elif strategy == "enhanced":
                    opposite_pos = self.get_enhanced_opposite_position(
                        agent.position, space
                    )
                elif strategy == "time_varying":
                    opposite_pos = self.get_time_varying_opposite_position(
                        agent.position, space, iteration, n_iterations
                    )
                else:  # elite
                    opposite_pos = self.get_elite_opposite_position(
                        agent.position, space
                    )
...
class ChaoticCDO(CDO):
    ...
    def update(
        ...
        for agent in space.agents:
            ...
            r1 = self.get_chaotic_value() * np.ones(
                (space.n_variables, space.n_dimensions)
            )
            r2 = self.get_chaotic_value() * np.ones(
                (space.n_variables, space.n_dimensions)
            )
            r3 = self.get_chaotic_value() * np.ones(
                (space.n_variables, space.n_dimensions)
            )
...
class MultiReactorCDO(CDO):
    ...
    def compile(self, space: Space) -> None:
        ...
        for _ in range(self.n_reactors):
            self.reactor_gamma_pos.append(
                np.zeros((space.n_variables, space.n_dimensions))
            )
            self.reactor_gamma_fit.append(c.FLOAT_MAX)
            self.reactor_beta_pos.append(
                np.zeros((space.n_variables, space.n_dimensions))
            )
            self.reactor_beta_fit.append(c.FLOAT_MAX)
            self.reactor_alpha_pos.append(
                np.zeros((space.n_variables, space.n_dimensions))
            )
    ...
    def exchange_information(self) -> None:
        ...
        # Find best reactor
        best_reactor = np.argmin(
            [self.reactor_alpha_fit[i] for i in range(self.n_reactors)]
        )
        ...
        for i in range(self.n_reactors):
            if i != best_reactor:
                if self.reactor_alpha_fit[best_reactor] < self.reactor_gamma_fit[i]:
                    self.reactor_gamma_pos[i] = copy.deepcopy(
                        self.reactor_alpha_pos[best_reactor]
                    )
    ...
    def update(
        ...
        for reactor in range(self.n_reactors):
            ...
            for agent in space.agents[start_idx:end_idx]:
                ...
                if (
                    fit < self.reactor_alpha_fit[reactor]
                    and fit < self.reactor_beta_fit[reactor]
                ):
                ...
                if (
                    fit < self.reactor_alpha_fit[reactor]
                    and fit < self.reactor_beta_fit[reactor]
                    and fit < self.reactor_gamma_fit[reactor]
                ):
            ...
            for agent in space.agents[start_idx:end_idx]:
                ...
                a_gamma = r3 * r3 * np.pi
                grad_gamma = np.abs(
                    a_gamma * self.reactor_gamma_pos[reactor] - agent.position
                )
                v_gamma = agent.position - rho_gamma * grad_gamma
                ...
                a_beta = r3 * r3 * np.pi
                grad_beta = np.abs(
                    a_beta * self.reactor_beta_pos[reactor] - agent.position
                )
                v_beta = 0.5 * (agent.position - rho_beta * grad_beta)
                ...
                a_alpha = r3 * r3 * np.pi
                grad_alpha = np.abs(
                    a_alpha * self.reactor_alpha_pos[reactor] - agent.position
                )
                v_alpha = 0.25 * (agent.position - rho_alpha * grad_alpha)
