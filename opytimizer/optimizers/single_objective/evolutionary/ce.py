...
class CE(Optimizer):
    ...
    def __init__(
        ...
        jump: int = 10,
        chaotic_system: _validSystems = "tent",
    ):
        """
            ...
            for j in range(space.n_variables):
                if (opt_r.generate_uniform_random_number() < self.CR) or (j == k):
                    if self.currentGen % self.jump == 0:
                        chaotic_agents[i].position[j] = space.best_agent.position[j] * (
                            1 + self.D[i][j] * self.cp[i][j]
                        )
                    else:
                        chaotic_agents[i].position[j] = chaotic_agents[i].position[
                            j
                        ] * (1 + self.D[i][j] * self.cp[i][j])
