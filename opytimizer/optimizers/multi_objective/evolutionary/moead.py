...
from opytimizer.math.aggregation import PBI, Tchebycheff, _BaseAggregation
...
class MOEAD(MultiObjectiveOptimizer):
    ...
    def __init__(
        ...
        Args:
            ...
            neighborhood_size: The size of agent's neighborhood.
            decomposition_method: The decomposition method reference (avaliable methods on `utils.math.aggregation`)
    ...
    def _build_neighborhood(self) -> None:
        dists = (
            np.sum(self.weight_vectors**2, axis=1, keepdims=True)
            - 2 * np.dot(self.weight_vectors, self.weight_vectors.T)
            + np.sum(self.weight_vectors**2, axis=1)
        )
        dists = np.maximum(dists, 0)
        neighbors = np.argpartition(dists, kth=self.neighborhood_size, axis=1)[
    ...
    def _genetic_operators(self, P1: Agent, P2: Agent) -> Union[Agent, List[Agent]]:
        ...
        if isinstance(offspring, tuple):
            ...
            child_1, child_2 = self.mutation_operator(
                child_1[0]
            ), self.mutation_operator(child_2[0])
    ...
    def _environmental_selection(
        self, children: List[Agent], subproblem_index: int, space: _MultiObjectiveSpace
    ) -> None:
        ...
        current_pop_aggr = self.decomposition_method(
            neighbors_fit, weights, np, z=self.z
        )
    ...
    def compile(self, space: _MultiObjectiveSpace):
        ...
        if self.neighborhood_size is None:
            self.neighborhood_size = int(0.2 * space.n_agents)
    ...
    def update(self, space: _MultiObjectiveSpace, function: Function) -> None:
        ...
        for sub_problem_index in subproblems_order:
            ...
            children = self._genetic_operators(
                space.agents[p_1_index], space.agents[p_2_index]
            )
...
class MOEADTensor(MultiObjectiveOptimizer, TensorizedMultiObjectiveOptimizer):
    ...
    def compile(self, space: _MultiObjectiveTensorSpace, **kwargs) -> None:
        ...
        if self.neighborhood_size is None:
            self.neighborhood_size = max(
                2, int(space.env.xp.ceil(self.n_subproblems * 0.2))
            )
    ...
    def _build_neighborhood(self, xp) -> None:
        diff = (
            self.weight_vectors[:, xp.newaxis, :]
            - self.weight_vectors[xp.newaxis, :, :]
        )
    ...
    def _mating_selection(self, space: _MultiObjectiveTensorSpace) -> Tuple:
        ...
        rand_idx = xp.random.randint(
            0, self.neighborhood_size, size=(self.n_subproblems, 2)
        )
    ...
    def update(self, space: _MultiObjectiveTensorSpace, function: Function) -> None:
        ...
        child1_X, child2_X = self.crossover_operator(
            parents1_X, parents2_X, space.lb, space.ub
        )
        del child2_X
...
class MOEAD_DE(MultiObjectiveOptimizer):
    ...
    def _DE_operator(
        ...
        mutant.position = parent1.position + self.F * (
            parent2.position - parent3.position
        )
        ...
        mutant.position = np.clip(
            mutant.position, mutant.lb.reshape(-1, 1), mutant.ub.reshape(-1, 1)
        )
    ...
    def _apply_operators(
        ...
        # Apply DE operator
        child = self._DE_operator(parent1=parent1, parent2=parent2, parent3=parent3)
    ...
    def _update_neighborhood(
        ...
        while c < self.nr and len(mating_pool) > 0:
            ...
            current_fitness = self.decomposition_function(
                current_agent.fit, self.weight_vectors[i], np, z=self.z
            )
    ...
    def update(self, space: _MultiObjectiveSpace, function: Function) -> None:
        ...
        for index, agent in enumerate(space.agents):
            ...
            parent2, parent3 = copy.deepcopy(space.agents[selected[0]]), copy.deepcopy(
                space.agents[selected[1]]
            )
