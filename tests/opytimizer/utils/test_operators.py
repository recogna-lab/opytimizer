...
def test_base_crossover_properties():
    ...
    assert crossover.n_offspring == 2

    crossover.rate = 0.5
    assert crossover.rate == 0.5

    crossover.n_offspring = 1
    assert crossover.n_offspring == 1
...
def test_arithmetic_crossover():
    ...
    assert isinstance(children[0][0], Agent) or isinstance(children[0][0], MockAgent)
    assert children[0][0].position.shape == (5, 1)
    assert np.all(children[0][0].position >= parent1.lb)
    assert np.all(children[0][0].position <= parent1.ub)

def test_sbx_crossover():
    parent1 = MockAgent(10, 0, 10)
    parent2 = MockAgent(10, 0, 10)
    
    crossover = SBXCrossover(rate=1.0, eta=20, n_offspring=1)
    children = crossover(parent1, parent2)
    ...
    new_crossover = SBXCrossover(n_offspring=2)
    new_children = new_crossover(parent1, parent2)

    assert len(new_children) == 2
    assert new_children[0][0].position.shape == (10, 1)
    assert np.all(new_children[1][0].position >= parent2.lb)
    assert np.all(new_children[1][0].position <= parent2.ub)

def test_sbx_crossover_n_offspring():
    parent1 = MockAgent(5)
    parent2 = MockAgent(5)
    
    crossover = SBXCrossover(n_offspring=1)
    children = crossover(parent1, parent2)
    assert len(children) == 1
    
    crossover.n_offspring = 2
    children = crossover(parent1, parent2)
    assert len(children) == 2
...
def test_one_point_crossover():
    ...
    crossover = OnePointCrossover(rate=1.0, n_offspring=1)
    children = crossover(parent1, parent2)
    ...
    new_crossover = OnePointCrossover(rate=1.0, n_offspring=2)
    new_children = new_crossover(parent1, parent2)

    assert len(new_children) == 2
    assert new_children[0][0].position.shape == (10, 1)
    assert new_children[1][0].position.shape == (10, 1)
    p1 = parent1.position.flatten()
    c1 = new_children[0][0].position.flatten()
    c2 = new_children[1][0].position.flatten()
    assert np.any(c1 == p1) 
    assert np.any(c2 == p1) 
